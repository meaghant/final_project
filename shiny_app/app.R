...
clean_data <- raw_data %>%
  select(ICPSR_CENTERID_PK, NUBOOKSLI_PK, RDDAYWKLI_PK, EDUCAT_PK, INCOMELI_PK, PALS_PK, TOTALASQSCORE_PK) %>%
  ...
  mutate(NUBOOKSLI_PK = as.character(NUBOOKSLI_PK),
         NUBOOKSLI_PK = fct_recode(NUBOOKSLI_PK, "None" = "1",
                                   ...
                                   "25 to 49 books" = "4",
                                   "50 or more books" = "5"),
         RDDAYWKLI_PK = as.character(RDDAYWKLI_PK),
...
nonschool_choices <- c("Number of Books in Home" = "NUBOOKSLI_PK",
                       "Number of Days Read to Per Week" = "RDDAYWKLI_PK",
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      tags$h6(helpText("\"Number of Books in Home\" refers to the total number of children's books, including library books, in the respondents' home at time of response.
                       \"Number of Days Read to Per Week\" refers to the number of days per week that parents typically read to their children.
    ...
    mainPanel(
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Explore the data", 
                           ...
                           p("My hypotheses largely held true. Generally, with increases in number of household books, maternal education level, and family income level, there was an improvement in both literacy and social development outcomes, although this plateaued somewhat at the higher levels of privilege in each factor. Suprisingly, there was not a substantial correlation between outcomes and number of books read to per week. I was also surprised by the fact that there was a stronger correlation between the three significant non-school factors and the literacy outcome variable than with the social development outcome variable. This is perhaps unexpected, given that all of these factors were removed from the classroom.")),
...
server <- function(input, output) {
  ...
  x_label <- reactive({
    ...
    } else if(input$nonschool == "RDDAYWKLI_PK"){
      x_label <- "Number of Days Read to Per Week"
  ...
  output$about <- renderUI({
    ...
    str3 <- paste("My Focus")
    str4 <- paste("I drew data from the Massachusetts Early Care and Education and School Readiness Study, which assesses the specific factors inside and outside of preschool classrooms that promote school readiness. I examined four non-school factors: number of books in the home, number of days read to per week, maternal education level, and family income level, and compared their effects on two outcome variables: literacy and social development. There was a stronger correlation between all four non-school factors and literacy outcomes than with social development outcomes. The substantial drop-offs in performance seen at the lowest levels of privilege on each non-school factor suggest a need for early education reform efforts directed towards those most in need.")
    str5 <- paste("Hypotheses")
    str6 <- paste("I hypothesized that preschool children who (1) are exposed to a greater number of books in their homes, (2) are read to a greater number of days per week, (3) have mothers with a higher degree of education, and (4) come from higher-income households will have better literacy and social development outcomes.")
  ...
  # Since I already had a good deal of explanatory text on screen, and I felt that it was clear to the viewer what was being displayed, I chose not to include a title directly on my boxplots.
  ...
  output$insight <- renderUI({
    ...
    str1 <- paste("Number of Books in the Home")
    ...
    str4 <- paste("Number of Days Read to Per Week")
    str5 <- paste("Surprisingly, I did not find a substantial correlation between number of days read to per week and either literacy or social development outcomes. There was a very slight improvement in social development scores, but too insignificant to be substantial. It seems that other non-school factors have more of an impact on preschoolers' outcomes.")
    str6 <- paste("Maternal Education Level")
    str7 <- paste("Maternal education level had an even stronger positive correlation with improved literacy outcomes. The spread of the data was consistent between educational levels, but the average literacy score improved with each additional level of school completed by the student’s mother.")
    str8 <- paste("Children of mothers who completed only high school or less were significantly less well behaved than children of other mothers. However, there were minimal differences in the social development of children whose mothers completed some college or received an associate’s, technical, bachelor’s or graduate degree.")
    str9 <- paste("Family Income Level")
    str10 <- paste("There was also a slight positive correlation between family income level and literacy outcomes. The largest gap was between families with incomes below $50,000 and all other families, whose children all exhibited similar literacy scores.")
    str11 <- paste("Finally, children of parents from lower-income families performed less well on scores of social development. Again, a gap emerged between families living on less than $50,000, whose children had the worst social development scores, and other families, whose children performed similarly to each other. Six of the seven worst-behaved children were in the lowest income bracket.")
    str12 <- paste("Implications")
    str13 <- paste("While there was no consistent correlation that emerged between outcomes and number of days read to per week, for most non-school factors, an increase in privilege drives an improvement in children's outcomes. However, above a certain threshold – whether number of books in the home, maternal education level, or family income level – children seem to perform at roughly the same level on literacy and social development outcomes. The substantial drop-offs in performance seen at the lowest levels of privilege on each factor might incline policymakers to focus non-school reform efforts towards those most in need.")
    
    HTML(paste(h3(str1), p(str2), p(str3), h3(str4), p(str5), h3(str6), p(str7), p(str8), h3(str9), p(str10), p(str11), h3(str12), p(str13)))
